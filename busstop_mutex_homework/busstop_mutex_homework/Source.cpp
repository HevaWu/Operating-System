...
int main()
{
	...
	//read in file
	ofstream outfile("output_busstop_simulation.txt");
	ifstream infile("initialization.txt");
	if (infile.fail())
	{
		std::cout << "Input file\"input.txt\"opening failed" << endl;
	}
	int busnumber, busstopnumber, betweenbus;
	infile >> busnumber >> busstopnumber >> boarding_time >> betweenbus;
	std::cout << busnumber << busstopnumber << boarding_time << betweenbus << endl;
	...
	do
	{
		...
		if (clock_judge == 1)
		{
			...
			outfile << "When clock is:" << clock << endl;
			now_clock(clock);
			//print the maximum/minimum/average of the waiting queue
			for (int j = 0; j <= 14; j++)
			{
				...
				std::cout << endl;
				outfile << "The maximum waiting queue of stop " << j + 1 << " is: " << bus_stop[j].maximum_queue << endl;
				outfile << "The minimum waiting queue of stop " << j + 1 << " is: " << bus_stop[j].minimum_queue << endl;
				outfile << "The average waiting queue of stop " << j + 1 << " is: " << bus_stop[j].average_queue << endl;
				outfile << endl;
		...
		if (clock_snapshot == 1)
		{
			...
			std::cout << "When clock is:";
			outfile << "Plot the positions of buses in " << freetime << " minutes." << endl;
			outfile << "When clock is:";
			...
			for (int i = 0; i <= 4; i++)
			{
				...
				if ((clock >= run_bus[i].before_begin_time) && (clock < run_bus[i].begin_time))
				{
					...
					outfile << "Bus " << i + 1 << " is on the road, which is between stop " << run_bus[i].stop_number << " and stop " << run_bus[i].stop_number + 1 << endl;
				}
				else
				{
					std::cout << "Bus " << i + 1 << " is at the stop  " << run_bus[i].stop_number + 1 << endl;
					outfile << "Bus " << i + 1 << " is at the stop  " << run_bus[i].stop_number + 1 << endl;
				}
